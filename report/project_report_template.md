...
- A limited cache storage budget at each edge server, equivalent to `C`
  average-size files
...
Let `S_f` be the size of file `f`, and let `S_0` be the nominal average file
size. The cache capacity constraint is:

```math
\sum_{f=1}^{N} S_f x_{k,f} \le C S_0, \quad \forall k.
```
...
Here, `S_{f_i}` is the requested file size, `T_bh` is fixed backhaul latency, and
`R_bh` is backhaul transmission rate. If the request is a cache hit, the backhaul
term is zero.
...
Random caching is used as a baseline. Each edge server randomly selects files
from the library until its storage budget is reached. This method does not use
content popularity or user location information.

### 5.2 Popularity-Based Caching

Popularity-based caching stores globally popular files at every edge server
under the cache storage budget. This strategy is simple and effective when the
popularity distribution is strongly skewed. However, it does not adapt to local
differences in user demand near different edge servers.
...
Each edge server then caches its own local top files under the cache storage
budget. This remains a simple heuristic, but it captures the idea that edge
servers may observe different local demand patterns.

### 5.4 Greedy Latency-Aware Caching

The greedy caching strategy estimates the backhaul latency saved by placing each
candidate file at each edge server. It then ranks candidate files by estimated
latency saving per Mbit and packs them under the cache budget. This makes the
policy local-demand-aware while remaining easy to understand and implement.

### 5.5 UCB-Style MAB Caching Extension

Describe the implemented learning baseline as a UCB-style combinatorial
semi-bandit rather than a novel reinforcement-learning algorithm. Each edge
server treats files as arms, selects a capacity-feasible cache once per training
epoch, and learns from the avoided backhaul delay of selected files. Explain the
reward-density score, fixed random seed, and chronological training/evaluation
split. Use only values generated by the dedicated comparison and multi-seed v2
scripts. Distinguish the Iteration 9 single-seed observation from the five-seed
mean, sample standard deviation, and paired difference results.
...
- Synthetic heterogeneous file sizes and fixed backhaul latency

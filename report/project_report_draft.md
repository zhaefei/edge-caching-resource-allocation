...
- Cache storage budget equivalent to `C` average-size files at each edge server
...
Each file has size `S_f`. Each edge server has a storage budget equivalent to
`C` average-size files, where `S_0` denotes the nominal average file size:
...
The default channel gain is computed using a deterministic path-loss model:
...
`d_min` is a minimum distance used to avoid singular values, and `eta` is the
path-loss exponent.

An optional reproducible fading snapshot is also available. It multiplies the
path-loss gain by a clipped sample from a unit-mean exponential distribution,
representing the power-gain form of a simplified Rayleigh fading model. This
extension is used for sensitivity analysis and does not model temporal channel
evolution.
...
Files are added until the storage budget is reached. It is used as a baseline
because it does not use content popularity or local demand information.

### 5.2 Global Popularity-Based Caching

Global popularity-based caching stores the most popular files at every edge
server according to the Zipf distribution while respecting the storage budget.
This strategy is simple and effective when the same popular content is requested
across the network.
...
local top files under the storage budget. This is still a simple heuristic, but
it captures the idea that different edge regions may observe different demand
patterns.

### 5.4 Greedy Latency-Aware Caching

The greedy strategy estimates how much backhaul latency can be avoided by
caching each candidate file at each edge server. It then ranks candidates by
estimated latency saving per cached Mbit and packs them under the storage
budget.
...
content requests. The default cache budget is equivalent to 8 average-size files
per edge server.
...
In the current default configuration, demand-aware bandwidth allocation improves
...
- File sizes are synthetic rather than based on measured traffic traces.
- The wireless channel uses simplified path loss, optional snapshot fading,
  and a coarse interference model.
...
- Adding mobility and time-correlated fading

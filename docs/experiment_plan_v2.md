...
- Results should examine how online selected-arm feedback compares with static
  ...
  assumed in advance.
- Iteration 9 produces a single-seed controlled comparison. The Iteration 10
  multi-seed v2 runner provides the corresponding cross-seed summary.

## 7. Multi-Seed v2 Summary Experiment

Implementation status: completed in Iteration 10 with fixed seeds 11, 22, 33,
44, and 55.
...
- Preserve every raw strategy/seed row and report sample standard deviation
  (`ddof=1`).
- Compute within-seed metric differences relative to random caching so topology
  variation is not confused with policy variation.
...
- A negative latency difference relative to random caching means lower latency;
  a positive hit-ratio difference means a higher hit ratio.
